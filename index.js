...
class Person {
  ...
  eat(food){if (this.stomach.length < 10) {
    this.stomach.push(food)} else {'Too Full!'}};
  poop(){this.stomach = [];};
...
class Car { constructor(model, milesPerGallon){
  ...
  drive(distance){
    ...
    if(distance < milesLeft){this.odometer = this.odometer + distance;this.tank = this.tank - gallonsUsed;} else { this.odometer = this.odometer + milesLeft;
      this.tank = 0;
